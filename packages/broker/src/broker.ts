...
import { BareHttp, BareHttpType } from 'barehttp';
        ...
        handler: async (flow) => {
          ...
          if (!services) {
            ...
            try {
              ...
              this.runtimeSchemaStore.set(gateway, [
                { name, schema: schemaToRegisterString, url, hash },
              ]);
            } catch (e) {
          ...
          const isSameService = services.find(
            ({ name: sName, url: sUrl, hash: sHash }) =>
              sName === name && sUrl === url && sHash === hash,
          );
          ...
          const findBreaking = diffed.filter(
            (d) => d.criticality.level === CriticalityLevel.Breaking,
          );
