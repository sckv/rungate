 ...
 */
export type Options =
  | ((request: Request, response: Response, params?: GraphQLParams) => MaybePromise<OptionsData>)
...
type Handler = (flow: BareRequest) => Promise<void>;

/**
 * Middleware for barehttp; takes an options object or function as input to
 * configure behavior, and returns an barehttp handler.
 */
export function graphqlHTTP(options: Options): Handler {
  devAssertIsNonNullable(options, 'GraphQL route handler requires options.');

  return async function graphqlMiddleware(flow: BareRequest): Promise<void> {
    const request = flow._originalRequest;
    const response = flow._originalResponse;
    ...
    try {
      ...
      try {
        params = await getGraphQLParams(flow);
      } catch (error: unknown) {
      ...
      if (query == null) {
        if (showGraphiQL) {
          return respondWithGraphiQL(flow, graphiqlOptions);
        }
      ...
      if (request.method === 'GET') {
        ...
        if (operationAST && operationAST.operation !== 'query') {
          ...
          if (showGraphiQL) {
            return respondWithGraphiQL(flow, graphiqlOptions, params);
          }
    ...
    if (flow.statusToSend === 200 && result.data == null) {
      flow.status(500);
    }
    ...
    if (showGraphiQL) {
      return respondWithGraphiQL(flow, graphiqlOptions, params, formattedResult);
    }
    ...
    if (!pretty && typeof flow.json === 'function') {
      flow.json(formattedResult);
    } else {
      const payload = JSON.stringify(formattedResult, null, pretty ? 2 : 0);
      sendResponse(flow, 'application/json', payload);
    }

    async function resolveOptions(requestParams?: GraphQLParams): Promise<OptionsData> {
      const optionsResult = await Promise.resolve(
        typeof options === 'function' ? options(request, response, requestParams) : options,
      );
 ...
 */
export async function getGraphQLParams(flow: BareRequest): Promise<GraphQLParams> {
  const urlData = new URLSearchParams(flow._originalRequest.url!.split('?')[1]);
  const bodyData = await parseBody(flow);
 ...
 */
function sendResponse(flow: BareRequest, type: string, data: string): void {
  flow.setHeader('Content-Type', type + '; charset=utf-8');
  flow.send(data);
}
